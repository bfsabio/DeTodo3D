...
window.onload = function() {
  ...
  cargarAjax('GET', 'http://pilote.techo.org/?do=api.getUnidadesOrganizacionales?idPais=1', 's-sede', selectSedes);
   ...
   document.getElementById('btn-submit').onclick = function() {
	 ...
	 var arrayElement=Array.from(form.elements);
	 var log;
	 arrayElement.pop();
	 arrayElement.forEach(function(element){
	     element.onfocus=function(){
	       log+="se hizo foco en "+element.name+"/h";
	     }
	     element.omblur=function(){
	       log+="se saco el foco de "+element.name+"/h";
	     }
	     element.onchange=function(){
	       log+="se cambio el valor de "+element.name+"/h";
	     }
